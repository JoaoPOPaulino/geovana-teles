...
export default function Agendamento({ slugSelecionado = '' }: AgendamentoProps) {
  ...
  // Inicializa o ouvinte de eventos do Cal.com
  useEffect(() => {
    (async function () {
      ...
      cal("on", {
        ...
        callback: (e) => {
          // Quando o cliente agenda com sucesso, o Cal.com avisa aqui!
  ...
  // Monta a URL e o link do WhatsApp
  const calLink = slugSelecionado 
    ? `${calUsername}/${slugSelecionado}` 
    : calUsername;

  // Monta o texto capturando a data do evento (se existir)
    ...
    <section id="agendamento" className="py-24 px-6 bg-white">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="rounded-2xl overflow-hidden border border-rose-100 bg-white">
          ...
          {!agendamentoConcluido ? (
            /* Componente Oficial que substitui o iframe */
            <Cal 
              key={calLink}
              calLink={calLink}
