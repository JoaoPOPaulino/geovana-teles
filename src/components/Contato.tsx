...
export default function Contato() {
  return (
    <section className="py-32 px-6 bg-white relative overflow-hidden">
      ...
      {/* Elemento decorativo */}
      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-64 h-64 rounded-full bg-rose-50 -translate-x-1/2" />
      ...
      <div className="max-w-4xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {infos.map((info) => (
            <div
              key={info.titulo}
              className="group border border-rose-100 rounded-2xl p-8 flex flex-col gap-4 hover:border-rose-300 hover:shadow-lg hover:shadow-rose-50 transition-all duration-300"
            >
              <div className="w-10 h-10 rounded-full bg-rose-50 flex items-center justify-center text-rose-400 group-hover:bg-rose-400 group-hover:text-white transition-all duration-300">
                {info.icone}
              </div>
              ...
              <div>
                ...
                {info.link ? (
                  <a
                    ...
                    className="font-medium text-gray-700 hover:text-rose-400 transition-colors"
                  >
                    {info.valor}
                  </a>
                ) : (
                  <p className="font-medium text-gray-700">{info.valor}</p>
                )}
                <p className="text-sm text-gray-400 mt-1 font-light">{info.descricao}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* CTA final */}
        <div className="relative rounded-3xl bg-rose-400 p-12 text-center overflow-hidden">
          <div className="absolute top-0 left-0 w-40 h-40 rounded-full bg-rose-300 opacity-30 -translate-x-1/2 -translate-y-1/2" />
          <div className="absolute bottom-0 right-0 w-56 h-56 rounded-full bg-rose-500 opacity-30 translate-x-1/3 translate-y-1/3" />

          <div className="relative">
            <h3 className="font-display text-3xl md:text-4xl font-medium text-white mb-4">
              Pronta para transformar sua pele?
            </h3>
            <p className="text-rose-100 font-light mb-8 max-w-md mx-auto">
